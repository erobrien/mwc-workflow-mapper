...
VARIABLES = {
  ...
  "methods": [
    {"name": "Method 1 - Custom Object (recommended)", "detail": "Treat each clinic as an independent object instance. Settings > Objects: create a 'Locations' Custom Object with fields Location Email, Location Phone, Booking Link, Address. Associate each Contact with their Location object at capture (WF-01). Because email/SMS builders cannot loop Custom Object variables natively, use an Update Contact Field action in the workflow to copy the associated Location's values into the Contact's operational fields right before a send."},
    {"name": "Method 2 - Contact Custom Field matrix", "detail": "Avoids Custom Objects. Create prefixed per-clinic fields on the Contact (LocA_Booking_Link, LocB_Booking_Link, LocA_Phone...). Tag the contact with its home clinic. A master If/Else workflow reads the location tag and copies the right clinic's value into a generic operational field (for example Current_Location_Booking_Link). Templates reference only the generic field. This is the encapsulation/polymorphism pattern inside GHL."},
    {"name": "Method 3 - Smart URL strategy (links only)", "detail": "For location-specific links only (booking, review, landing page): one dynamic base Custom Value plus a contact field appended as a URL parameter, for example {{custom_values.base_booking_url}}?branch={{contact.location_preference}}. Landing-page JavaScript reads ?branch= and renders the correct clinic calendar/details. Good for redirects, not for in-message clinic data like address or phone."}
  ],
  "custom_values": [
