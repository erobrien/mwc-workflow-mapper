...
import { useCodyInventory } from "../lib/cody";
import { Database, Tag, Waypoints, ClipboardList, CalendarCheck, Braces } from "lucide-react";

export default function CodyInventory() {
  const { data, isLoading } = useCodyInventory();
  ...
  return (
    <PageShell
      title="Cody build — inventory"
      subtitle={`Fields, tags, pipelines, custom values, calendars, and forms captured live from the build sub-account (${data.location_id}).`}
    >
