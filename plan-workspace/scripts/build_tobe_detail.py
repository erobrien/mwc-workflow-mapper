...
VARIABLES = {
  ...
  "collision_warning": "CRITICAL: a standard GHL Custom Value is account-wide. It applies universally across the whole sub-account, so it CANNOT hold per-clinic data. If one location writes a Custom Value it changes for every location (a data collision). Custom Values are only safe for values that are identical brand-wide (brand name, main brand domain, compliance links). Per-clinic values must NEVER live in a Custom Value.",
  "methods": [
    {"name": "Method 1 - Locations Custom Object (LOCKED: Decision 14)", "detail": "The chosen pattern. A 'Locations' Custom Object with one record per clinic (fields: Location Email, Location Phone, Booking Link, Address, Review Link). Each Contact is associated with its Location at capture, and WF-01 stamps the associated Location's values into generic operational contact fields (current_clinic_address, current_clinic_phone, current_booking_url) exactly once at creation. Because email/SMS builders cannot loop Custom Object variables natively, that stamp step is the bridge; every template reads ONLY the generic fields, so downstream workflows never know clinics exist. Wave 0 addendum: the object + 3 records + generic fields must be created (Wave 0 ran before Decision 14)."},
    {"name": "Method 2 - Contact field matrix (REJECTED)", "detail": "Prefixed per-clinic fields plus tag-routed If/Else copying into generic fields. Rejected by Decision 14 because the clinic values end up embedded inside workflow branch configs, which buries data in steps (the defect this project removes) and makes every clinic-data change a workflow edit."},
    {"name": "Method 3 - Smart URL strategy (ADOPTED for links)", "detail": "Layered on per Decision 14 for booking/review links on the Vercel funnel pages: one base Custom Value plus the contact's location as a URL parameter, for example {{custom_values.base_booking_url}}?branch={{contact.location_preference}}. The landing page reads ?branch= and renders the correct clinic calendar. Links only; in-message address/phone comes from the generic fields."}
  ],
  "custom_values": [
    "Safe as account-wide Custom Values ({{custom_values.name}}): brand name, main brand booking domain, compliance links, brand-level review/survey/intake links. These are identical for every contact and every clinic.",
    "NOT safe as Custom Values (per-clinic, would collide): clinic address, clinic direct phone, per-clinic booking/calendar URL, location email. Deliver these via Method 1 or 2: copy the clinic's value into a generic operational Contact field with an Update Contact Field step before the send, then reference that generic field.",
    "Merge-field syntax: contact fields {{contact.field_key}}, opportunity fields {{opportunity.field_key}}, account constants {{custom_values.name}}, appointment {{appointment.start_time}}. Operational pattern: templates read a generic field like {{contact.current_location_booking_link}} that the workflow fills per clinic, never a hardcoded clinic value."
  ],
}
...
WF["01"] = {
  ...
  "prerequisites": [
    ...
    "Wave 0 ADDENDUM (Decision 14): Locations Custom Object with 3 records (Richmond, Virginia Beach, Newport News) + generic operational contact fields current_clinic_address, current_clinic_phone, current_booking_url",
